...
import upo.additionalstructures.WorkStruct;
...
public class Greedy {
	...
	public static Integer[] getMaxDisjointIntervals(Integer[] starting, Integer[] ending) {
		// throw new UnsupportedOperationException("Not yet implemented");
		//sort 
		WorkStruct[] works = new WorkStruct[ending.length];
		for(int i = 0; i< ending.length;i++) {
			works[i] = new WorkStruct(starting[i], ending[i], i);
		}
		
		for (int i = 0; i < ending.length; i++) {
			for (int j = i + 1; j < ending.length; j++) {
				WorkStruct tmp;
				if (works[i].getEndTime() > works[j].getEndTime()) {
					tmp = works[i];
					works[i] = works[j];
					works[j] = tmp;
					tmp = works[i];
		...
		int t = works[0].getEndTime();
		List<Integer> sol = new ArrayList<Integer>();
		sol.add(works[0].getPosition());
		
		for(int i=1; i<ending.length; i++) {
			if(works[i].getStartTime() >= t) {
				t=works[i].getEndTime();
				sol.add(works[i].getPosition());
			}	
		}
		Integer[] retArray = new Integer[sol.size()];
		return sol.toArray(retArray);
	}
